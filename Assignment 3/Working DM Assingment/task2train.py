...
model_file = sys.argv[2] #Model file
stopwords_file = sys.argv[3] #Stop words file


main_rdd = sc.textFile(input_file) #Loading the Input file
rdd = main_rdd.map(json.loads) #Converting to JSON

business_doc = rdd.map(lambda x: (x["business_id"],x["text"])).reduceByKey(lambda x,y: x+y)
...
words_count = words_in_doc.count()

words_index = words_in_doc.reduceByKey(lambda x,y:x+y).map(lambda x: x[0]).distinct().zipWithIndex()
words_dict = words_index.map(lambda x : {x[0]:x[1]}) #Counting the number of words for rare_words calculation
...
unique_word_count = business_words.flatMap(unique_word_cal).reduceByKey(lambda x,y : x+y)
...
def bus_prof_gen(x):
  ...
  top = top[:200]
  top_index_words = [x[0] for x in top]
  return tuple([business_id, top_index_words]) #Business with top index words
