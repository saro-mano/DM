  ...
  if len(chunks):
    for chunk in chunks:
      ...
      for candidate in candidate_items:
          d[candidate] += 1
    output = list(d.items()) #Returning (C,v)
  return output
...
def main():
  ...
  '''Transforming main_rdd into user_id : [business_id] based on the case number'''
  if case_number == 1:
    rdd = main_rdd.filter(lambda x: x != header).map(lambda x: tuple([x.split(",")[0],[x.split(",")[1]]])).reduceByKey(lambda x,y: list(set(x+y)))
  else:
    rdd = main_rdd.filter(lambda x: x != header).map(lambda x: tuple([x.split(",")[1],[x.split(",")[0]]])).reduceByKey(lambda x,y: list(set(x+y)))
